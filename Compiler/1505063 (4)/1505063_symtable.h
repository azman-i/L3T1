    ...
    hash_Map(int num)
{
  ...
  Num=num;
   ...
   //  cout<<list[i]->value<<endl;
    ...
    void insert_item(string str,int key,int value,string namestr,string type,bool isfunc)
{
      cout<<str<<endl;
   //  cout<<"bal"<<endl;
    int key_real=key;
...
struct hash_Map* temp;
    ...
    return temp;
     ...
     bool InsertCurrent(string str,string namestr,string type,bool isfunc)
{
cout<<cur->Num<<endl;
    int key=cur->hash_function1(str);
    cout<<"hello1"<<endl;
    cur->insert_item(str,key,cur->count,namestr, type, isfunc);
return false;
}
